...
export default function ProfilePage() {
  ...
  const initiateSecurityUpdate = (type: 'email' | 'password') => {
    ...
    if (type === 'password' && newPassword.length < 6) {
  ...
  const handleSecurityAction = async () => {
    ...
    try {
      ...
      await signOut(auth);
      router.push('/login');
      toast({ title: "Security Refresh", description: "Please sign in with your new credentials." });
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="space-y-6">
            <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden text-center p-8">
            ...
            <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-4">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                As an <strong>{profile?.role}</strong>, you have access to specialized workspace controls. Keep your credentials secure to protect organizational data.
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-8">
            <Card className="border-none shadow-sm bg-white rounded-3xl p-8">
              <CardHeader className="px-0 pt-0">
                ...
                <CardDescription>Update your public workspace name.</CardDescription>
              </CardHeader>
              <CardContent className="px-0 space-y-6">
                ...
                <Button onClick={handleUpdateName} className="w-full h-12 rounded-xl font-bold">
                  Update Identity
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="border-none shadow-sm bg-white rounded-3xl p-8">
              <CardHeader className="px-0 pt-0">
                <CardTitle className="text-xl font-bold font-headline">Security & Credentials</CardTitle>
                <CardDescription>Update your sensitive authentication details.</CardDescription>
              </CardHeader>
              <CardContent className="px-0 space-y-8">
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-widest">Account Email</Label>
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" onClick={() => initiateSecurityUpdate('email')} className="h-12 rounded-xl px-6 border-2 font-bold">
                        Change
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t space-y-4">
                    ...
                    <Button onClick={() => initiateSecurityUpdate('password')} className="w-full h-12 rounded-xl font-bold bg-secondary text-secondary-foreground hover:bg-secondary/80">
                      Update Password
                    </Button>
                  </div>
                </div>
      ...
      <Dialog open={isReauthOpen} onOpenChange={setIsReauthOpen}>
        <DialogContent className="rounded-[2.5rem] p-8 max-w-md">
          <DialogHeader className="text-center">
            ...
            <DialogDescription>
              To update your sensitive account details, please confirm your current password.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="pt-4 flex-col gap-2">
            <Button onClick={handleSecurityAction} className="w-full h-14 rounded-2xl font-bold" disabled={isSubmitting}>
              {isSubmitting ? "Verifying..." : "Confirm & Update"}
            </Button>
            <Button variant="ghost" onClick={() => setIsReauthOpen(false)} className="w-full rounded-2xl">
              Cancel
            </Button>
          </DialogFooter>
        </DialogContent>
